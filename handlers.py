...
DRAW_PROMPT_INSTRUCTION = (
    "用简体中文写成一句完整、连贯、尽可能详细的图片描述，"
    "自然包含主体数量与身份、外貌特征、服饰道具、姿态动作、表情情绪、场景环境、构图视角、镜头距离、光线色彩、材质纹理、画面风格、氛围基调和需要保留或避免的关键细节，"
    "不要分点、不要换行、不要使用英文。"
)
...
async def draw(
    ...
    Examples:
        ...
        draw("一只毛色蓬松的橘猫蜷缩在老木窗台上的浅蓝色棉垫里安静打盹，猫咪闭着眼睛露出放松表情，尾巴轻轻搭在前爪旁，窗外午后阳光穿过半透明白纱窗帘洒在猫背和绿植叶片上，背景能看到摆着陶瓷杯、小书堆和玻璃花瓶的温馨房间，画面采用近景三分构图、柔和暖黄色光线、细腻水彩插画风格、纸张纹理清晰、氛围宁静治愈且不要出现文字水印。", "1024x720")

        # Modify existing image and send to chat
        draw("保留参考图中人物的发型、脸部特征、服装轮廓和主要姿势，将背景改成春日盛开的樱花公园小径，人物站在飘落花瓣之间微微回头微笑，远处有木质长椅、浅色路灯和被柔焦处理的游客剪影，画面使用半身偏全身构图、轻微仰视镜头、清透粉白色调、柔和逆光、细腻动漫插画风格，整体氛围浪漫清新且不要改变主体身份。", "1024x1024", "shared/refer_image.jpg", send_to_chat=_ck) # if adapter supports file, you can use this method to send the image to the chat. Otherwise, find another method to use the image.
    """
    # 生成图片
...
async def generate_openai_multi_image_api(
    ...
    for index, image_ref in enumerate(multi_request.reference_images, 1):
        weight_hint = f"（重要程度: {image_ref.weight}）" if image_ref.weight != 1.0 else ""
        prompt_lines.append(f"参考图片{index}: {image_ref.description}{weight_hint}")
    prompt_lines.append(f"目标描述: {multi_request.target_prompt}")
    prompt_lines.append(
        f"生成尺寸为 {multi_request.size} 的最终图片，并在一句尽可能详细的简体中文描述中保留参考图的重要视觉特征。",
    )
...
async def generate_chat_mode_image(model_group, prompt: str, size: str, refer_image: str, source_image_data: str) -> str:
    """使用聊天模式绘图"""
    system_content = f"你是一名专业画师，请严格根据用户提供的图片描述绘制图片；图片描述必须遵循要求：{DRAW_PROMPT_INSTRUCTION}只提供生成的图片，不要追问更多信息。"
        ...
        if not config.USE_SYSTEM_ROLE:
            msg = msg.add(
                ContentSegment.text_content(
                    f"{system_content}\n\n请仔细分析上方参考图片，并根据以下描述绘制图片：{prompt}（尺寸：{size}）",
                ),
            )
        else:
            msg = msg.add(
                ContentSegment.text_content(
                    f"请仔细分析上方参考图片，并根据以下描述绘制图片：{prompt}（尺寸：{size}）",
                ),
            )
    else:
        if not config.USE_SYSTEM_ROLE:
            msg = msg.add(
                ContentSegment.text_content(
                    f"{system_content}\n\n请根据以下描述绘制图片：{prompt}（尺寸：{size}）",
                ),
            )
        else:
            msg = msg.add(
                ContentSegment.text_content(f"请根据以下描述绘制图片：{prompt}（尺寸：{size}）"),
            )
...
async def generate_multi_image(
    ...
    # 构建多图聊天消息
    system_content = f"你是一名擅长分析并融合多张视觉参考图的专业画师，请根据目标描述创作新图片；图片描述必须遵循要求：{DRAW_PROMPT_INSTRUCTION}"
